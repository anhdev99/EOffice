...
import {loaiVanBanModel} from "@/models/loaiVanBanModel";
...
export default {
  ...
  data() {
    return {
      ...
      submitted: false,
      model: loaiVanBanModel.baseJson(),
      listCoQuan: [],
      listRole: [],
  ...
  watch: {
    ...
    showModal(status) {
      if (status == false) this.model = loaiVanBanModel.baseJson();
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("loaiVanBanStore/getById", id).then((res) => {
        console.log("LOG HANDLE UPDATE ", id)
        if (res.resultCode === 'SUCCESS') {
          this.model = loaiVanBanModel.fromJson(res.data);
          this.showModal = true;
        } else {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("loaiVanBanStore/delete", this.model.id).then((res) => {
          if (res.resultCode === 'SUCCESS') {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("loaiVanBanStore/update", this.model).then((res) => {
          ...
          // Create model
          await this.$store.dispatch("loaiVanBanStore/create", loaiVanBanModel.toJson(this.model)).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
              this.showModal = false;
              this.$refs.tblList.refresh()
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("loaiVanBanStore/getPagingParams", params)
              ...
              <div class="col-sm-8">
                <div class="text-sm-end">
                  <b-button
                      ...
                      size="sm"
                  >
                    <i class="mdi mdi-plus me-1"></i> Tạo loại văn bản
                  </b-button>
                  <b-modal
                      v-model="showModal"
                  ...
                  >
                    <form @submit.prevent="handleSubmit"
                      ...
                      <div class="row">
                        <div class="col-12">
                          <div class="mb-3">
                            <label class="text-left">Tên loại văn bản</label>
                            ...
                            >
                              Tên loại văn bản không được để trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-12">
                  ...
                  <b-modal
                      v-model="showDetail"
                      title="Thông tin chi tiết loại văn bản"
                      title-class="text-black font-18"
                      body-class="p-3"
                      hide-footer
                      centered
