...
<template>
  <div class="container-fluid">
    ...
    <template v-else>
      <ul id="side-menu" class="navbar-nav h-100">
        <template v-for="item of menuItems">
          <li class="menu-title" v-if="item.isTitle" :key="item.id">
            <span data-key="t-menu">{{ $t(item.label) }}</span>
          </li>
          <li class="nav-item" v-if="!item.isTitle && !item.isLayout" :key="item.id">
            <a v-if="hasItems(item)" :href="`#sidebar${item.id}`" class="nav-link menu-link"
               data-bs-toggle="collapse"
               role="button"
               aria-expanded="false"
...
<!--              <span :class="`badge rounded-pill bg-${item.badge.variant} float-end`" v-if="item.badge">{{ $t(item.badge.text) }}</span>-->
            ...
            <router-link :to="item.link" v-if="!hasItems(item)" class="nav-link menu-link">
              <i :class="`${item.icon}`" v-if="item.icon"></i>
              <span>{{ $t(item.label) }}</span>
<!--              <span :class="`badge rounded-pill bg-${item.badge.variant} float-end`" v-if="item.badge">{{ $t(item.badge.text) }}</span>-->
            </router-link>

            <div v-if="hasItems(item)" class="collapse menu-dropdown" :id="`sidebar${item.id}`">
              <ul class="nav nav-sm flex-column">
                <li class="nav-item" v-for="(subitem, index) of item.subItems" :key="index">
                  <router-link :to="subitem.link" v-if="!hasItems(subitem)" class="nav-link">{{ $t(subitem.label) }}</router-link>
                </li>
              </ul>
            </div>
